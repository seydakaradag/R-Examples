...
#install.packages("Ecdat")    ## install the package from CRAN
library(Ecdat)               ## load the library (package)
data(Computers)              ## load the data set

str(Computers)               ## structre of data frame
dim(Computers)               ## dimensions: number of rows and columns
summary(Computers)           ## general summary of all variables
names(Computers)

mean(Computers$price)        ## descriptive statistics:
median(Computers$price)
var(Computers$price)
sd(Computers$price)
range(Computers$price)
...
cor(Computers$price, Computers$ram)

cor(Computers[c("price", "speed", "ram", "hd")])